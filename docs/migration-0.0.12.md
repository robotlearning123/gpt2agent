...
The current installation's GET surface cannot prove that the App has no other
...
release prepares and validates the complete GitHub draft before PyPI, then
revalidates that exact draft after the public-package canary. An intervening
mutation blocks GitHub publication but cannot roll back PyPI.
